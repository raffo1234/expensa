...
export default function GeneratePDFButtonInner({
  ...
  useEffect(() => {
    if (!instance.loading && instance.url && downloadLinkRef.current) {
  ...
  return (
    <Link
      ...
      title={label}
      target="_blank"
      className="flex gap-2 items-center text-white cursor-pointer disabled:opacity-90 py-2 px-6 bg-rose-400 hover:bg-rose-500 transition-colors duration-500 rounded-full"
    >
      {instance.loading ? (
        <IconLoading />
      ) : (
