...
import FallBackResidents from "./FallBackResidents";
...
export default function ResidentsPageContent({
  ...
  userId: string;
  residentsFallback: UserType | null;
}) {
  const { data, isLoading } = useSWR("admin-residents", () => residentsFetcher(userId), {
    fallbackData: residentsFallback,
  });
  const residents = data?.residents;

  if (isLoading) return <FallBackResidents />;
  ...
  return (
    <>
      {data?.residents?.length === 0 ? (
        ...
        >
          {residents?.map(({ first_name, last_name, id, role_id, role, image_url }) => {
            return (
              <div
                key={id}
